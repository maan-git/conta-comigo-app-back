...
class Command(BaseCommand):
    ...
    neighborhood_queryset = Neighborhood.objects.all().select_related(
        "city", "city__state"
    )
    city_queryset = City.objects.all().select_related("state")
    address_queryset = Address.objects.all().select_related(
    ...
    def handle(self, *args, **options):
        ...
        if not self.should_import():
            print(
                "The last importation date is bigger than the addresses date. It was already imported"
            )
        else:
            ...
            update_global_variable_value(
                "LAST_ADDRESSES_UPDATE", self.addresses_datetime
            )
    ...
    def should_import(self):
        ...
        return (
            last_addresses_update_date is None
            or last_addresses_update_date < self.addresses_datetime
        )

    def read_all_csv_lines(self) -> List[str]:
        if not os.path.exists(self._current_csv_file_path):
            raise CommandError(
                'File "{}" not found'.format(self._current_csv_file_path)
            )
    ...
    def get_addresses_date(self):
        ...
        if not os.path.exists(addresses_date_file):
            raise CommandError(
                'Addresses date file "{}" does not exists'.format(addresses_date_file)
            )
    ...
    def _load_neighborhood_cache(self):
        ...
        [
            self.add_neighborhood_in_cache(neighborhood)
            for neighborhood in self.neighborhood_queryset
        ]
    ...
    def _get_state_from_cache(self, uf_initials: str) -> dict:
        return [
            state for state in self.states_cache if state.get("initials") == uf_initials
        ][0]
    ...
    def get_city_from_cache(self, description: str, state_id: int) -> {dict, None}:
        try:
            return [
                ...
                if city.get("description") == description
                and city.get("state_id") == state_id
            ][0]
        except IndexError:
            return None

    def get_neighborhood_from_cache(
        self, description: str, city_id: int
    ) -> {dict, None}:
        try:
            return [
                neighborhood
                for neighborhood in self.neighborhood_cache
                if neighborhood.get("description") == description
                and neighborhood.get("city_id") == city_id
            ][0]
        except IndexError:
            return None
    ...
    def _load_city(self, state: dict, city_description: str) -> dict:
        ...
        if city_from_cache is None:
            ...
            city, created = self.city_queryset.get_or_create(
                **city_data, defaults=city_data
            )
            if created:
                ...
                if self._verbosity_level >= self.VerbosityLevels.created_objects:
                    print(
                        'Created city "{}" in state "{}"'.format(
                            city.description, state.get("initials")
                        )
                    )
    ...
    def _load_neighborhood(self, city: dict, neighborhood_name: str) -> dict:
        neighborhood_from_cache = self.get_neighborhood_from_cache(
            neighborhood_name, city.get("id")
        )
        ...
        if neighborhood_from_cache is None:
            ...
            neighborhood, created = self.neighborhood_queryset.get_or_create(
            ...
            if created:
                ...
                if self._verbosity_level >= self.VerbosityLevels.created_objects:
                    error_message_mask = (
                        'Created neighborhood "{}" in city "{}" and state "{}"'
                    )
                    print(
                        error_message_mask.format(
    ...
    def _load_address(
        self, neighborhood: dict, address_text: str, zip_code: str
    ) -> Address:
        address_data = {
        ...
        address, created = self.address_queryset.get_or_create(
            **address_data, defaults=address_data
        )

        if self._verbosity_level >= self.VerbosityLevels.created_objects and created:
            address_log_mask = (
                'Created address "{}" in neighborhood "{}", city "{}" and state "{}"'
            )
            print(
                address_log_mask.format(
                    address.description,
    ...
    def extract_csvs_from_zip(self):
        with zipfile.ZipFile(self._input_file_path) as zip_arch:
            ...
            if csv_files is None or len(csv_files) == 0:
                raise CommandError(
                    "The input file is a zip file, but it doesn't contain CSV files"
                )

            for csv_file in csv_files:
                try:
                    zip_arch.extract(csv_file, os.path.dirname(self._input_file_path))
                    self._csv_files_path.append(
                        os.path.join(os.path.dirname(self._input_file_path), csv_file)
                    )
                except Exception as e:
                    raise CommandError(
                        'Extract of file "{}" from zip has failed.', csv_file
                    )
    ...
    def get_city_dict(cls, csv_line: str) -> {dict, None}:
        ...
        city_name, address_text, neighborhood_name, uf_initials, zip_code = (
            address_fields
        )
    ...
    def import_csv_file(self, csv_file):
        ...
        for index, line in enumerate(file_lines):
            try:
                ...
                if not city_data:
                    raise CommandError(
                        'Adress data "{}" in line "{}" is not valid'.format(
                            line, index + 1
                        )
                    )
                ...
                neighborhood = self._load_neighborhood(
                    city, city_data.get("neighborhood_name")
                )

                self._load_address(
                    neighborhood,
                    city_data.get("address_text"),
                    city_data.get("zip_code"),
                )
                ...
                raise CommandError(
                    error_mask.format(index + 1, self._current_csv_file_path, e)
                )
        if self.is_zip_file:
